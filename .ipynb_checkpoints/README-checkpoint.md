...
And finally, you need to create the "model_define.hpp" file just as I have created it. To do this, you should define your neural network model using the esp-dl libraries. The only potentially challenging part here is entering the input parameters for each of the layers. To do this, you simply need to correctly enter all the exponents generated by the Python program for each layer. These exponents are actually powers of two that, when multiplied by the model parameters, can reconstruct the initial model parameters created in Python. Essentially, these coefficients are created after quantizing the generated initial model parameters. Also, make sure to manually change the input and output size of your model in this file, and set it to the same value as in the original model. If the input and output size in this file do not match the original model, your project will not run correctly.
